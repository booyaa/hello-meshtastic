...
def on_connection(interface, topic=pub.AUTO_TOPIC):
    ...
    print(f"Connected to {long_name} (ID: {id}), Battery Level: {battery_level}%")
    # import pprint
    # pprint.pprint(my_node_info)
...
def on_receive(packet, topic=pub.AUTO_TOPIC):
    ...
    if packet_type == "TEXT_MESSAGE_APP":
        ...
        print(f"Sender details: {from_long_name} ({from_short_name}) ID: {from_id} Hardware: {from_hw}")
        if to == 'broadcast':
            channel = packet.get('channel', 'DEFAULT')
            print(f"Received: {message} from {from_node} on channel {channel}")
        else:
            print(f"Received: {message} (DM) from {from_node} to {'me' if to == my_node_info['num'] else to}")
            # FIXME: need to get this data out of this method
...
def main():
    ...
    try:
        ...
        print("we come to the end...")
        interface.close()



if __name__ == "__main__":
    print("Starting Meshtastic TCP client...")
    main()
